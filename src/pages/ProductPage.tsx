...
const slugToHref: Record<string, string> = {
  "smart-switch": "/products/smart-switch",
  "led-white": "/products/led-white",
  "led-rgb": "/products/led-rgb",
};
...
const ProductPage = () => {
  ...
  if (!href) return <Navigate to="/products" replace />;

  const product = products.find((p) => p.href === href);
  if (!product) return <Navigate to="/products" replace />;

  const media = productMedia[href];
  const heroGraphic = isProductHeroImage(href);
